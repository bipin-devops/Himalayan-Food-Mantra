@extends('layout')
@section('title') List of Product @endsection
@section('content')
        ...
        <div class="panel-heading">
            ...
            <div class="row">
                <ul class="icons-list pull-right">
                    <li>
                        <a class="btn btn-success" href="{{URL::to('system/product/create')}}"><i
            ...
            <div class="content-display clearfix">
                <div class="panel panel-default">
                    <div class="panel-heading no-bdr">
                        <div class="row">
                            <div class="col-sm-6">
                                {!!Form::open(['method'=>'GET','url'=>'system/product', 'class'=>'form-inline'])!!}
                ...
                @else
                    ...
                    @foreach($datas as $datum)
                        ...
                        <tr>
                            ...
                            @if($datum->image)
                                <td>
                                    <img src="{{ $datum->imageUrl }}"
                                         alt="{{ $datum->title }}" width="85">
                                </td>
                            @else
                                <td>    <img id="employeeImage" src="{{ asset('admin/images/no_img.png') }}" alt="your image"
                                             class="preview-image" style="size:50px"/></td> @endif
                            <td>{{$datum->category->name ?? ''}}</td>


                            <td>{{$datum->title}}</td>
                            <td data>{{$datum->price}}</td>

                            <td>

                                <a class="btn btn-sm btn-info btn_glyph"
                                   href="{{URL::to('system/product/edit/'.$datum->id)}}"><i
                                            class="glyphicon glyphicon-edit"></i> {{('Edit')}}</a>
                                <a class="btn btn-sm btn-danger btn_glyph"
                                   onclick="return confirm(' you want to delete?');"
                                   href="{{URL::to('system/product/delete/'.$datum->id)}}"><i
